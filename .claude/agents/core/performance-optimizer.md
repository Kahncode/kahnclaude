...
description: Use PROACTIVELY when users report slowness, high cloud costs, or scaling concerns, or before traffic spikes. Identifies bottlenecks, profiles workloads, and applies optimizations for blazingly fast systems.
...
3. **Fix the Top Bottlenecks**
   ...
   - Keep code readable; avoid premature micro-optimization.
...
- **Caching**: memoization, HTTP caching, DB result cache.
- **Concurrency**: async/await, goroutines, thread pools.
- **Query Optimization**: indexes, joins, batching, pagination.
