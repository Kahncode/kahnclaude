<#
.SYNOPSIS
    Triggers Start Debugging (F5) in the running Visual Studio instance.
    Builds if needed, then launches the editor with the debugger attached.
...
# Find the VS instance with our solution loaded
$dte = $null
foreach ($obj in [ROTHelperLaunch]::GetDTEInstances()) {
    try {
...
Write-Host "Starting debug session: $configName"

# Equivalent of pressing F5 - builds if needed, then launches with debugger
$dte.ExecuteCommand("Debug.Start")

Write-Host "Debug session launched. Monitor progress in VS Output window."
